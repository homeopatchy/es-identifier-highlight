...
const grammarScopes = ['source.ts', 'source.js', 'source.js.jsx', 'source.babel', 'source.js-semantic'];
    ...
    constructor() {
        ...
        this._subscriptions.add(
            atom.commands.add('atom-workspace', {
              'ts-identifier-highlight:renameIdentifier': () => this._handleRefactorHotkey()
            })
        );

        this._subscriptions.add(
            atom.commands.add('atom-workspace', {
              'ts-identifier-highlight:prevIdentifier': () => this._handlePrevIdHotkey()
            })
        );

        this._subscriptions.add(
            atom.commands.add('atom-workspace', {
              'ts-identifier-highlight:nextIdentifier': () => this._handleNextIdHotkey()
            })
        );

        this._subscriptions.add(
            atom.commands.add('atom-workspace', {
              'ts-identifier-highlight:jumpToDefinition': () => this._handleDefinitionHotkey()
            })
        );
    ...
    _mark(start, end, classes) {
        ...
        if (this._minimapActive) {
            this._createMarker(
                ...
                range,
                atom.config.get('ts-identifier-highlight.minimapLine') ? 'line' : 'highlight',
                classes
            );
        }
    }
